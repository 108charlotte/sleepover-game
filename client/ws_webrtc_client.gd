...
func connect_to_url(url: String) -> void:
	print("[ws_webrtc_client] connect_to_url -> %s" % url)
	close()
	code = 1000
	reason = "Unknown"
...
func close() -> void:
	print("[ws_webrtc_client] close() called; current state: %d" % ws.get_ready_state())
	ws.close()
...
func _process(_delta: float) -> void:
	...
	if state != old_state:
		print("[ws_webrtc_client] state change: %d -> %d" % [old_state, state])
	...
	if state != old_state and state == WebSocketPeer.STATE_CLOSED:
		# Capture last close information from the peer and emit disconnected.
		code = ws.get_close_code()
		reason = ws.get_close_reason()
		print("[ws_webrtc_client] connection closed; code=%d, reason='%s'" % [code, reason])
		disconnected.emit()
	old_state = state
